...
function useController(props) {
  ...
  const _proxyRef = import_react.default.useRef(null);
  ...
  const onChange = import_react.default.useCallback((event) => {
...
function useFieldArray(props) {
  ...
  import_react.default.useMemo(() => !disabled && rules && fields.length >= 0 && control.register(name, rules), [control, name, fields.length, rules, disabled]);
  useIsomorphicLayoutEffect(() => {
    if (disabled) {
      return;
    }
  ...
  return {
    swap: import_react.default.useCallback(swap, [updateValues, name, control, disabled]),
    move: import_react.default.useCallback(move, [updateValues, name, control, disabled]),
    prepend: import_react.default.useCallback(prepend, [
      updateValues,
      name,
      control,
      disabled
    ]),
    append: import_react.default.useCallback(append, [updateValues, name, control, disabled]),
    remove: import_react.default.useCallback(remove, [updateValues, name, control, disabled]),
    insert: import_react.default.useCallback(insert$1, [updateValues, name, control, disabled]),
    update: import_react.default.useCallback(update, [updateValues, name, control, disabled]),
    replace: import_react.default.useCallback(replace, [
      updateValues,
      name,
      control,
      disabled
...
function Form(props) {
  ...
  const handleSubmit = import_react.default.useMemo(() => {
  ...
  return import_react.default.createElement("form", { noValidate: mounted, action, ...!isFunction(action) && { method, encType }, onSubmit: submit, ...rest }, children);
}
var FormState = ({ control, disabled, exact, name, render }) => render(useFormState({ control, name, disabled, exact }));
...
function collectDirtyFieldNames(dirtyTree, cachedDirtyFields, prefix = "", names = []) {
  for (const key in dirtyTree) {
    ...
    if (isDirtyContainer(value) && isDirtyContainer(get(cachedDirtyFields, path))) {
    ...
    if (!_options.disabled || shouldDirty === true) {
      if (!isBlurEvent || shouldDirty) {
        ...
        if (isCurrentFieldPristine !== _formState.isDirty) {
          updateDirtyFields(_formState.dirtyFields, getDirtyFields(_defaultValues, _formValues, void 0, _fields));
        } else {
    ...
    if (!keepStateOptions.keepValues) {
      if (keepStateOptions.keepDirtyValues) {
        const fieldsToCheck = /* @__PURE__ */ new Set([
          ..._names.mount,
          ...collectDirtyFieldNames(getDirtyFields(_defaultValues, _formValues, void 0, fieldRefs), _formState.dirtyFields)
        ]);
...
function useForm(props = {}) {
  ...
  const _formControlProp = import_react.default.useRef(props.formControl);
  const [formState, updateFormState] = import_react.default.useState(() => ({
    ...cloneObject(DEFAULT_FORM_STATE),
...
//# sourceMappingURL=chunk-LOZLF5SG.js.map
